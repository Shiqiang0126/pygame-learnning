...
salary = []
disabled_button_element = '<button.*soupager__btn">下一页</button>'
disabled_button = None
p = 1

while not disabled_button:
    url = 'https://sou.zhaopin.com/?p=' + str(p) + '&jl=530&kw=python工程师&kt=3&sf=0&st=0'
    print('正在爬取第' + str(p) + '页:' + url)
    session = HTMLSession()
    ...
    # 判断页面中下一页按钮还能不能点击
    disabled_button = not re.findall(disabled_button_element, page.html.html)
